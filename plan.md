...
- [authz] の error log が [object Object] となり、認可系 DB 例外の原因特定が困難だった
...
- assertServiceAssignment の error log を JSON.stringify で構造化し、code/message/details/hint/stack を必ず出力
...
6. [authz] のログが [object Object] にならず、error.code / error.message が確認できること
...
## ?? �C�����O�iPhase 3 HTTP �G���[�n���h�����O���P�j

### Fix: PGRST116 �}�b�s���O + service_code �Q�ƏC��

**�Ǐ�**  
- `/services/life-care/users` GET `/api/care-receivers?serviceId=life-care` �� 404 ��Ԃ�
- care_receivers �e�[�u���� facility_id �łȂ� service_code �ōi�荞�ޕK�v������
- �G���[���O�� `[object Object]` ���\�������iString(err) �̎g�p�j

**����**  
1. **HTTP �}�b�s���O���**: service_staff.single() �� "no rows" (PGRST116) �� 404 �ŕԂ��Ă���
   - ���ۂɂ́u�����Ȃ��v�������_���I�G���[ �� 403 ��������
   - 404 �� resolveService�iservice ������Ȃ��j�ŕԂ��ׂ�
2. **care_receivers �t�B���^�����O���**: facility_id �ōi�荞��ł���
   - �������� services.code ���擾���� care_receivers.service_code �ōi�荞��
   - hasFacilityId �t�H�[���o�b�N�����݂���]�n��������
3. **���O���x�����**: String(err) �ɂ�� Vercel Logs �� `[object Object]` ���\��

**�Ή���**  
1. **assertServiceAssignment() �̏C��** [commit: f8d9850]
   - PGRST116�ino rows�j�� 403 "Access denied"�i�����Ȃ��j
   - ���̑� PGRST �� 404 "Service not found"�iservice ������Ȃ��j
   - �S catch �� structured JSON: `{ code, message, details, hint, stack }`

2. **care-receivers endpoint �̏C��** [commit: 07f9381]
   - service_code �Q�Ƃɓ���iresolvedSlug �ł͂Ȃ� services.code ���擾�j
   - facility_id �֘A�̏��������S�폜�iselect, where �����j
   - �ԋp���X�|���X�� serviceCode ��ǉ��iserviceSlug �Ƃ̕��p�j

3. **���O�S�p�~**
   - String(err) �����ׂĔp�~
   - resolveServiceIdToUuid, assertServiceAssignment, care-receivers �S�n���h���[�� structured logging

**���؁iPreview�j**  
- `/services/life-care/users` �ɃA�N�Z�X ?
- GET `/api/care-receivers?serviceId=life-care` �X�e�[�^�X: 200 ?
- ���X�|���X: `{ ok: true, careReceivers: [...], count, serviceSlug, serviceCode }`  

**�e���͈�**
- `/api/care-receivers` �G���h�|�C���g�icare receivers �ꗗ�擾�j
- lib/authz/serviceScope.ts�i�S API �Ŏg�p�j
- Vercel Logs �̉ǐ�����i[object Object] �p�~�j

**���[���o�b�N�v��**  
1. `git revert f8d9850` + `git revert 07f9381`
2. �܂��� `git reset --hard HEAD~2`  
3. Preview: `/services/life-care/users` �� 404/403 �ɖ߂�z��
