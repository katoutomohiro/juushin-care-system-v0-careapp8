...
test.describe('Seizure History Page', () => {
  test('should display seizure history page', async ({ page }) => {
    await page.goto('/daily-log/seizure/history')
    
    // ページタイトルの確認
    await expect(page.getByRole('heading', { name: '発作記録 履歴' })).toBeVisible()
  ...
  test('should filter seizure logs by date range', async ({ page }) => {
    await page.goto('/daily-log/seizure/history')
  ...
  test('should export CSV', async ({ page }) => {
    ...
    expect(filename).toMatch(/seizure_logs_\d+\.csv$/)
  })
  ...
  test('should navigate to new seizure log form', async ({ page }) => {
    await page.goto('/daily-log/seizure/history')
    ...
    // 発作記録ページに遷移すること
    await expect(page).toHaveURL('/daily-log/seizure')
    await expect(page.getByRole('heading', { name: '発作記録' })).toBeVisible()
  })
})
