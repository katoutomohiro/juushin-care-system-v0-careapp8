...
**Expected Behavior**:
1. `ux-ready` label added 竊・ci-automerge workflow triggered
2. Workflow monitors both required checks
3. When both reach SUCCESS 竊・Auto squash merge + delete branch
...
**笞・・Warning**: Document reason in PR comment before using admin override.
...
while ((Get-Date) -lt $deadline) {
    ...
    if ($allSuccess) {
        Write-Host "笨・All required checks passed for PR #$PrNumber"
        return $true
    }
    
    Write-Host "竢ｳ Waiting for checks... (${TimeoutMinutes}m timeout)"
    Start-Sleep -Seconds 30
}

Write-Host "笶・Timeout waiting for checks"
return $false
```
...
**Steps**:
1. SonarCloud.io 竊・Project Settings 竊・Analysis Method 竊・Disable "Automatic Analysis"
...
**Trade-offs**:
- 笨・Full control over scan configuration
- 笨・Coverage reporting and gating
- 笶・More complex workflow maintenance
- 笶・Longer analysis time
...
**System Status**: 泙 Production Ready  
